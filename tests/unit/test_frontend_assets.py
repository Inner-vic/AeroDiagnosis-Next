...
def test_frontend_exposes_focused_knowledge_qa_and_progressive_root_cause_flow() -> None:
    ...
    assert 'id="hybridQuery"' not in html
    assert "多路召回分析台" not in html
    ...
    assert "/retrieval/hybrid" not in script
    assert "runHybridRetrieval" not in script
    assert "融合相关度" not in script
