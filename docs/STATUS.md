...
- 已构建 `public-aero-corpus-v1.1`：10 份公开文档或数据源、17 个原始文件、536 个确定性文本块；
- 来源扩展到 NASA、FAA、NTSB、Federal Register 和 Europe PMC，覆盖 PDF、TXT、HTML、XML、JSON、CSV 六类媒体格式；
- 构建器支持官方来源下载、字节数与 SHA-256 校验、PDF 加密检查、JATS 表格解析、跨格式对照及离线复验；
- 已形成 16 个解析样本、24 条知识抽取候选和 30 个检索问题，全部显式标记为非金标准；
- NASA 与 Federal Register 的跨格式 token overlap 已记录为退化检测基线，不冒充人工解析准确率；
- FAA SDR 样本经发动机系统筛选、JASC 分层和直接标识字段移除；撤稿、禁止演绎和许可不明确的候选载荷被质量门禁排除。
