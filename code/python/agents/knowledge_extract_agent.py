...
class KnowledgeExtractAgent:
    ...
    def _parse_response(self, raw: str, source_id: str) -> ExtractionResult:
        try:
            ...
            return ExtractionResult(entities=[], relations=[], source_chunk_id=source_id)

        entities = [
            Entity(name=e.get("name", ""), type=e.get("type", "Concept"), description=e.get("description", ""))
            for e in data.get("entities", []) if e.get("name")
        ]
        relations = [
            Relation(head=r.get("head", ""), relation=r.get("relation", "related_to"),
                     tail=r.get("tail", ""), confidence=float(r.get("confidence", 0.5)))
            for r in data.get("relations", []) if r.get("head") and r.get("tail")
        ]
        return ExtractionResult(entities=entities, relations=relations, source_chunk_id=source_id)
    ...
    def _deduplicate(results: list[ExtractionResult]) -> list[ExtractionResult]:
        ...
        for result in results:
            ...
            deduped.append(ExtractionResult(
                entities=unique_entities, relations=unique_relations,
